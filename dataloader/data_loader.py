...
class CMPDataIter(data.IterableDataset):
    ...
    def __init__(self, data_root, data_set, max_rul, seq_len):
        ...
        self.regr = linear_model.LinearRegression() # feature of linear coefficient
        ...
        self.train_x, self.train_ops, self.train_handcraft, self.train_y, self.test_x, self.test_ops, self.test_handcraft, self.test_y = self._process(self.train_data_df, self.test_data_df, self.test_truth)
        ...
        logging.info("CMPDataIter:: iterator initialized (train moving handcraft shape: {:})".format(len(self.train_handcraft)))
        ...
        logging.info("CMPDataIter:: iterator initialized (test moving handcraft shape: {:})".format(len(self.test_handcraft)))
        logging.info("CMPDataIter:: iterator initialized (test label shape: {:})".format(len(self.test_y)))

        self.folded_train_x, self.folded_train_ops, self.folded_train_handcraft, self.folded_train_y = \
                            self.cross_fold([self.train_x, self.train_ops, self.train_handcraft, self.train_y])
    ...
    def _process(self, train_df, test_df, test_truth):
        ...
        for train_idx, train in grouped_train:
            scaled_train = minmax_scaler.fit_transform(train.iloc[:, 3:])
            ...
            for test_idx, test in grouped_test:
                if train_idx == test_idx:
                    scaled_test = minmax_scaler.transform(test.iloc[:, 3:])
        ...
        train_setting = minmax_scaler.fit_transform(train_df.iloc[:, 1:5])
        test_setting = minmax_scaler.transform(test_df.iloc[:, 1:5])
        ...
        handfeat_pth = './hand_feat'
        if not os.path.exists(handfeat_pth):
            os.makedirs(handfeat_pth)
        handfeat_data_pt = os.path.join(handfeat_pth, "handfeat_train_{:}.pkl".format(self.data_set))
        if os.path.exists(handfeat_data_pt):
            logging.info("CMPDataIter:: load pre-computed handcradfted features from {:}".format(handfeat_data_pt))
            train_handcraft = pickle.load(open(handfeat_data_pt, 'rb'))
        else:
            logging.info("CMPDataIter:: generating train handcrafted data, this may cost several minutes...")
            train_handcraft = []
            for i in range(len(train_x)):
                data_temp = train_x[i]
                train_handcraft.append(self.handcrafted_feat(data_temp))
                if (i+1) % 100 == 0:
                    logging.info("CMPDataIter:: train handcrafted data had been generated {:}/{:}".format(i+1, len(train_x)))
            with open(handfeat_data_pt, 'wb') as f:
                pickle.dump(train_handcraft, f)
            logging.info("CMPDataIter:: save the computed handcrafted features at {:}".format(handfeat_data_pt))
        logging.info("CMPDataIter:: train handcrafted data generation finished")
        ...
        handfeat_data_pt = os.path.join(handfeat_pth, "handfeat_test_{:}.pkl".format(self.data_set))
        if os.path.exists(handfeat_data_pt):
            logging.info("CMPDataIter:: load pre-computed handcradfted features from {:}".format(handfeat_data_pt))
            test_handcraft = pickle.load(open(handfeat_data_pt, 'rb'))
        else:
            logging.info("CMPDataIter:: generating test handcrafted data, this may cost several minutes...")
            test_handcraft = []
            for i in range(len(test_x)):
                data_temp = test_x[i]
                test_handcraft.append(self.handcrafted_feat(data_temp))
                if (i+1) % 100 == 0:
                    logging.info("CMPDataIter:: test handcrafted data had been generated {:}/{:}".format(i+1, len(test_x)))
            with open(handfeat_data_pt, 'wb') as f:
                pickle.dump(test_handcraft, f)
            logging.info("CMPDataIter:: save the computed handcrafted features at {:}".format(handfeat_data_pt))
        logging.info("CMPDataIter:: test handcrafted data generation finished")
        ...
        logging.info("CMPDataIter:: finished")

        return train_x, train_ops, train_handcraft, train_y, test_x, test_ops, test_handcraft, test_y
    ...
    def handcrafted_feat(self, data): # feature extraction of two features
        ...
        for i in range(data.shape[0]-1):
            for j in range(data.shape[1]):
                ...
                self.regr.fit(x[:(i+2)].reshape(-1,1),np.ravel(data[:(i+2),j]))
                fea[i, 2*j+1] = self.regr.coef_
    ...
    def reset(self, mode):
        ...
        if mode == 'train':
            ...
            self.cross_train_handcraft = train_handcraft[0] + train_handcraft[1] + train_handcraft[2] + train_handcraft[3] 
            ...
            self.out_handcraft = self.cross_train_handcraft
            ...
            self.out_handcraft = self.cross_val_handcraft
            ...
            self.out_handcraft = self.test_handcraft
    ...
    def cross_fold(self, data_list):
        #input: self.train_x, self.train_ops, self.train_handcraft, self.train_y
        ...
        train_x, train_ops, train_handcraft, train_y = zip(*zip_list)

        grouped_train_x = []
        grouped_train_ops = []
        grouped_train_handcraft = []
        grouped_train_y = []
        ...
        for g_id in range(4):
            ...
            group_train_handcraft = train_handcraft[0+g_id*group_size:(g_id+1)*group_size]
            ...
            grouped_train_handcraft.append(group_train_handcraft)
        ...
        grouped_train_handcraft.append(train_handcraft[4*group_size:])
        grouped_train_y.append(train_y[4*group_size:])

        return grouped_train_x, grouped_train_ops, grouped_train_handcraft, grouped_train_y
    ...
    def __iter__(self):
        ...
        out_handcraft = self.out_handcraft[self.start: self.end]
        out_y = self.out_y[self.start: self.end]

        sum_iter = zip(out_x, out_ops, out_handcraft, out_y)
